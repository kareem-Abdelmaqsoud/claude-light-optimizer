...
def optimize_wavelength_cli():
    parser = argparse.ArgumentParser(description='Optimize Claude-light RGB values for a target wavelength using Gemini API or LHS.')
    parser.add_argument('--wavelength', type=str, required=True, help='The target wavelength to optimize (e.g., 515nm).')
    parser.add_argument('--iterations', type=int, default=5, help='Number of optimization iterations for Gemini or samples for LHS.')
    parser.add_argument('--strategy', type=int, choices=[1, 2, 3, 4], help='Optimization strategy: 1=Gemini, 2=LHS, 3=Compare, 4=Exit')
    ...
    if args.strategy:
        ...
        print("3. Compare Gemini vs. LHS") # New option
        print("4. Exit")
    ...
    if strategy_choice == '1':
        ...
        print(f"\nFinal Best RGB found: R={best_rgb[0]:.2f}, G={best_rgb[1]:.2f}, B={best_rgb[2]:.2f}")
        print(f"Highest {target_wavelength}nm output: {best_output}")
    elif strategy_choice == '3': # Comparison option
        if gemini_model is None:
